...
namespace MySqlSideKicks.Win
{
    public partial class SessionForm : Form, ISessionView
    {
        ...
        private void InitializeSyntaxHighlighting()
        {
            ...
            // CALL
            codeScintilla.Styles[Style.Sql.User2].BackColor = Color.Purple;
            codeScintilla.Styles[Style.Sql.User2].ForeColor = Color.LightYellow;
            codeScintilla.Styles[Style.Sql.User2].Bold = true;
            codeScintilla.Styles[Style.Sql.User2].Underline = true;
            ...
            codeScintilla.Indicators[HighlightIndicator].ForeColor = Color.Lime;            
            codeScintilla.Indicators[HighlightIndicator].OutlineAlpha = 255;
            codeScintilla.Indicators[HighlightIndicator].Alpha = 30;

            // Word = 0
            codeScintilla.SetKeywords(0, @"start mediumint interval month day year prepare duplicate elseif ignore out integer leave add alter as authorization backup begin bigint binary bit break browse bulk by cascade case catch check checkpoint close clustered column commit compute constraint containstable continue create current cursor cursor database date datetime datetime2 datetimeoffset dbcc deallocate decimal declare default delete deny desc disk distinct distributed double drop dump else end errlvl escape except exec execute exit external fetch file fillfactor float for foreign freetext freetexttable from full function goto grant group having hierarchyid holdlock identity identity_insert identitycol if image index insert int intersect into key kill lineno load merge money national nchar nocheck nocount nolock nonclustered ntext numeric nvarchar of off offsets on open opendatasource openquery openrowset openxml option order over percent plan precision primary print proc procedure public raiserror read readtext real reconfigure references replication restore restrict return revert revoke rollback rowcount rowguidcol rule save schema securityaudit select set setuser shutdown smalldatetime smallint smallmoney sql_variant statistics table table tablesample text textsize then time timestamp tinyint to top tran transaction trigger truncate try union unique uniqueidentifier update updatetext use user values varbinary varchar varying view waitfor when where while with writetext xml go ");
            // Word2 = 1
            codeScintilla.SetKeywords(1, @"last_insert_id ifnull left right sum trim least mid date_sub count now concat length replace ascii cast char charindex ceiling coalesce collate contains convert current_date current_time current_timestamp current_user floor isnull max min nullif object_id session_user substring system_user tsequal ");
            // User1 = 4
        ...
        private void codeScintilla_UpdateUI(object sender, UpdateUIEventArgs e)
        {
            ...
            {
                var identifier = codeScintilla.SelectedText;

                if (identifier == string.Empty)
                {
                    identifier = codeScintilla.GetWordFromPosition(codeScintilla.CurrentPosition);
                }
        ...
        public void HighlightText(string text)
        {
            ...
            codeScintilla.SearchFlags = SearchFlags.WholeWord;
